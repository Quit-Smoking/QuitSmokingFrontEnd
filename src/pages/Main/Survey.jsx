                ...
                <div className="Survey-Page Page0InputCenter">
                    <div className="Input">
                        {isEditing ?
                            (
                                // 편집 중일 때는 input으로 표시
                                <input ref={howManyref} className="editable-input" type="number" value={howMany} onChange={HowManyChange} onBlur={handleBlur} autoFocus />
                            ) :
                            (
                                // 편집 중이 아닐 때는 텍스트로 표시
                                <div className="editable-text" onClick={handleFocus}> {howMany || "몇 개비 피우셨나요?"} {/* 입력값 없을 시 기본 문구 */}
                                </div>
                            )}
                    </div>
                </div>
                        ...
                        {isEditing ? (
                            ...
                            <div>
                                <input
                                    ...
                                    type="text"
                                    value={motive}
                                    onChange={MotiveChange}
                                    onBlur={handleBlur} // 포커스를 벗어나면 편집 모드 종료
                                    autoFocus
                                />
                            </div>
                        ) : (
                            ...
                            <div
                                ...
                                onClick={handleFocus}
                            >
                                {motive || "금연 다짐을 입력해주세요"} {/* 입력값 없을 시 기본 문구 */}
                            </div>
                        )}
                    </div>
                </div>
            ...
            <div className="Survey-Container">
                ...
                <div className="Survey-Footer">
                    <div className={`Footer-Grid-box ${page === 0 ? "firstNextbtn" : ""}`}>
                        {
                            page != 0 ? <div className="box previousbtn" onClick={() => pageMinus()}>이전</div> : null
                        }
                        {
                            {
                                0:
